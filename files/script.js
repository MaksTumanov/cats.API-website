document.addEventListener('DOMContentLoaded', function () {
    const apiUrl = 'https://cats.petiteweb.dev/api/single/mix_tumaster';
        ...
        catInfoContainer.innerHTML = `
            <div class="cat-info">
                <div class="cat-details">
                    <h3>${cat.name}</h3>
                    <h4>Age: ${getAgeString(cat.age || 'Не указан')}</h4>
                    <p>${cat.description || 'Описание отсутствует'}</p>
                </div>
                <div class="cat-image">
                    <img src="${cat.image || 'files/images/default.jpg'}" alt="${cat.name}">
                </div>
            </div>
        `;
        ...
        fetch(`${apiUrl}/show`)
            ...
            .then(cats => {
                ...
                cats.forEach(cat => {
                    ...
                    catCard.innerHTML = `
                        <img src="${cat.image || 'files/images/default.jpg'}" alt="Котик" style="width: 100%; height: auto;">
                        <h3>${cat.name}</h3>
                        <div>
                            <button onclick="showEditCatModal(${cat.id})"><i class="fa-solid fa-pencil"></i></button>
                            <button onclick="showViewCatModal(${cat.id})">More info</button>
                            <button onclick="deleteCat(${cat.id})"><i class="fa-solid fa-ban"></i></button>
                        </div>
                    `;
